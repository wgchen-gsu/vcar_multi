...
from keras.layers import Add, Input, Add, PReLU, Conv2DTranspose, Concatenate, MaxPooling2D, UpSampling2D, Dropout, Activation
...
import cv2
from common import SubpixelConv2D
...
def get_unet_model(input_channel_num=3, out_ch=1, start_ch=64, depth=4, inc_rate=2., activation='relu',
    ...
    im_input = Input(shape=(None, None, input_channel_num))
    map_input = Input(shape=(None, None, start_ch))
    
    o = _level_block(im_input, start_ch, depth, inc_rate, activation, dropout, batchnorm, maxpool, upconv, residual)
    u = UpSampling2D(size=(2,2))(map_input)
    o = Concatenate()([u, o])
    o = Conv2D(out_ch, 3, padding='same')(o)

    model = Model(inputs=[im_input, map_input], outputs=o)
